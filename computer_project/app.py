...
monthly_data = combined_df.groupby(['location', 'Year-Month'], as_index=False).agg({
    'total_cases': 'sum',
    'total_deaths': 'sum'
})
...
combined_df.to_csv("monthly_cleaned_data.csv", index=False)
def generate_plot():
    ...
    pl.title("Total Cases and Total Deaths Over Time")
    ...
    img = io.BytesIO()
    pl.savefig(img, format='png', bbox_inches="tight")
    img.seek(0)
